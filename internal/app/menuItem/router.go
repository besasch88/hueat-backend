...
func (r menuItemRouter) register(router *gin.RouterGroup) {
	...
	router.POST(
		"/menu/tables/:tableId/categories/:menuCategoryId/items",
		hueat_auth.AuthMiddleware([]string{hueat_auth.READ_MENU, hueat_auth.WRITE_MY_TABLES}),
		hueat_timeout.TimeoutMiddleware(time.Duration(1)*time.Second),
		func(ctx *gin.Context) {
			// Input validation
			var request createCustomMenuItemInputDto
			if err := hueat_router.BindParameters(ctx, &request); err != nil {
				hueat_router.ReturnValidationError(ctx, err)
				return
			}
			...
			// Business Logic
			item, err := r.service.createCustomMenuItem(ctx, request)
