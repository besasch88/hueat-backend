...
func (s menuItemService) createMenuItem(ctx *gin.Context, input createMenuItemInputDto) (menuItemEntity, error) {
	...
	newMenuItem := menuItemEntity{
		...
		MandatoryForInside:  hueat_utils.BoolPtr(false),
		MandatoryForOutside: hueat_utils.BoolPtr(false),
		TableID:             nil,
		CreatedAt:           now,
		UpdatedAt:           now,
	}
		...
		if event, err := s.pubSubAgent.Persist(tx, hueat_pubsub.TopicMenuItemV1, hueat_pubsub.PubSubMessage{
			Message: hueat_pubsub.PubSubEvent{
				...
				EventEntity: &hueat_pubsub.MenuItemEventEntity{
					...
					MandatoryForInside:  newMenuItem.MandatoryForInside,
					MandatoryForOutside: newMenuItem.MandatoryForOutside,
			...
			if event, err := s.pubSubAgent.Persist(tx, hueat_pubsub.TopicMenuItemV1, hueat_pubsub.PubSubMessage{
				Message: hueat_pubsub.PubSubEvent{
					...
					EventEntity: &hueat_pubsub.MenuItemEventEntity{
						...
						MandatoryForInside:  updatedEntity.MandatoryForInside,
						MandatoryForOutside: updatedEntity.MandatoryForOutside,
...
func (s menuItemService) updateMenuItem(ctx *gin.Context, input updateMenuItemInputDto) (menuItemEntity, error) {
	...
	errTransaction := s.storage.Transaction(func(tx *gorm.DB) error {
		...
		if input.MandatoryForInside != nil {
			updatedMenuItem.MandatoryForInside = input.MandatoryForInside
		}
		if input.MandatoryForOutside != nil {
			updatedMenuItem.MandatoryForOutside = input.MandatoryForOutside
		}
		if input.Price != nil {
		...
		if event, err := s.pubSubAgent.Persist(tx, hueat_pubsub.TopicMenuItemV1, hueat_pubsub.PubSubMessage{
			Message: hueat_pubsub.PubSubEvent{
				...
				EventEntity: &hueat_pubsub.MenuItemEventEntity{
					...
					MandatoryForInside:  currentMenuItem.MandatoryForInside,
					MandatoryForOutside: currentMenuItem.MandatoryForOutside,
