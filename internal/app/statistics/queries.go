...
FROM hueat_course_selection AS s
INNER JOIN hueat_course AS c
ON s.course_id = c.id
INNER JOIN hueat_order as ord
ON c.order_id = ord.id
INNER JOIN hueat_table as t
ON ord.table_id = t.id
INNER JOIN hueat_menu_item AS i
ON s.menu_item_id = i.id
LEFT JOIN hueat_menu_option AS o
